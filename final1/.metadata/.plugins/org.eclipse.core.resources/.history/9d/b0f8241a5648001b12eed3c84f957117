...
<div style="margin:0 auto;text-align:center;margin-top:40px;"><strong style="font-size:28px;#EC7777">${cvo.cname}</strong><span style="font-size:16px;">&nbsp;에대한 솔직한 리뷰를 작성해주세요.</span></div>

	<form name="frm" action="reviewUpdate" method="post"
		enctype="multipart/form-data">
		<div style="width:678px;height:591px;margin:0 auto;text-align:center;padding: 16px;">
		<div style="border:1px solid gray;margin:10px;padding:10px;">
		<input type="hidden" name="ccode" value="${cvo.ccode}">
		<input type="hidden" name="rcode" value="${rvo.rcode}">
		<input type="hidden" name="rscope" value="${rvo.rscope}">
			<p id="star_grade">
       	 		<a href="#" style="font-size:30px;" id="star1">★</a>
        		<a href="#" style="font-size:30px;" id="star2">★</a>
        		<a href="#" style="font-size:30px;" id="star3">★</a>
        		<a href="#" style="font-size:30px;" id="star4">★</a>
        		<a href="#" style="font-size:30px;" id="star5">★</a>
			</p>
			
			<textarea maxlength="10000" name="rcontent" style="border: 0;resize: none;margin:10px auto;width:100%;overflow: hidden; overflow-wrap: break-word; height: 150px;padding:10px;">${rvo.rcontent}</textarea> 
		</div>
		<div>
			<div id = "listFile" style="overflow:hidden;margin:5px;">
			<c:forEach items="${images}" var="image">
				<img src="display?fileName=attach/${image}">
			</c:forEach>
			...
			<input type = "reset" value = "취소" style="float:right;    min-width: 140px;min-height: 50px;margin-right: 16px;padding-left: 14px;padding-right: 14px;border: 1px solid #7F7F7F;border-radius: 50px;font-size: 16px;color: #7F7F7F;text-align: center;background-color: #FFFFFF;" onClick="location.href='read?ccode=${cvo.ccode}'">
		</div>
		</div>
	</form>
    <!-- ##### Footer Area Start ##### -->
...
<script>
var rscope=$(frm.rscope).val();
for(var i=1;i<=rscope;i++){
	$("#star"+i).addClass("on");
}
