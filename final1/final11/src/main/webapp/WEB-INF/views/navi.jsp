...
<c:if test="${fn:length(lastview) !=0}">

<div id=lastviewbox onClick="location.href='read?ccode=${lastview[size-1].ccode}'">


<c:if test="${lastview[size-1].cimage!='' }">
<div style="width:90px;height:90px;"><img src="display?fileName=cafeimage/${lastview[size-1].cimage}"/></div>
</c:if>
...
<div><a style="font-size:40px;float:left;margin-left:10px; margin-right:30px" ><img src="${path}/resources/img/core-img/x.png" width=25 height=25></a><h2 class = "lastviewlist" style=" margin-left:30%;margin-top:15px;" >최근 본 카페 </h2></div>
<c:forEach var="i" begin="1" end="${size}">
	<div id=lastviewbox onClick="location.href='read?ccode=${lastview[size-i].ccode}'">
<c:if test="${lastview[size-i].cimage!='' }">
<span style="width:150px;height:150px;margin-left:3%;"><img src="display?fileName=cafeimage/${lastview[size-i].cimage}" style="width:100px;height:100px; "></span>
</c:if>
...
<style>
#lastviewlist{width:20%;height:65%;
max-height:65%;
overflow-y:auto;
 position: fixed;
 right:0px;
